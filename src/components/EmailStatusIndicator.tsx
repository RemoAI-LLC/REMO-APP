        ...
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center space-x-3">
            <div className="w-10 h-10 bg-green-100 rounded-full flex items-center justify-center">
              <FaEnvelopeOpenText className="text-green-600 text-lg" />
            </div>
            <div>
              <h3 className="text-lg font-semibold text-gray-900">Gmail Connected</h3>
              <p className="text-sm text-gray-600">Email integration active</p>
            </div>
          </div>
        </div>
            ...
            <div className="space-y-1">
              {authStatus.scopes.includes('https://www.googleapis.com/auth/gmail.modify') && (
                <div className="flex items-center space-x-2">
                  ...
                  <span className="text-xs text-gray-600">Send & manage emails</span>
                </div>
              )}
              {authStatus.scopes.includes('https://www.googleapis.com/auth/calendar') && (
                <div className="flex items-center space-x-2">
                  <div className="w-2 h-2 bg-green-500 rounded-full"></div>
                  <span className="text-xs text-gray-600">Calendar access</span>
                </div>
              )}
              {authStatus.scopes.includes('https://www.googleapis.com/auth/calendar.events') && (
                <div className="flex items-center space-x-2">
                  <div className="w-2 h-2 bg-green-500 rounded-full"></div>
                  <span className="text-xs text-gray-600">Schedule meetings</span>
                </div>
              )}
            </div>
          </div>
